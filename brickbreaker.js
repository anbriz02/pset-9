...
var canvas;
	...
	function init() {
		...
		BALL_WIDTH = 20;
		BALL_HEIGHT = 20;
		ball = new Ball(canvas.width / 2, canvas.height/ 2, BALL_WIDTH, '#D6EAF8');
	...
	function loop() {
		...
		if(lives >= 0) {
			...
			ctx.fillStyle = 'black';
			ctx.font = '64px Arial';
			ctx.fillText('Hahaha, try again', 200, 300);
			ctx.font = '18px Arial';
			ctx.fillText('Press the R key to start again', 200, 380);
